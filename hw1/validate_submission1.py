...
def get_submitters(temp_folder):    
    ...
    if not os.path.join(temp_folder, 'my_module.h'):
	raise Exception('No vigenere_module.h file - Invalid format')
    if not os.path.join(temp_folder, 'my_module.c'):
	raise Exception('No vigenere_module.c file - Invalid format')
